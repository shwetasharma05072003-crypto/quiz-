...
# -----------------------
# RAW QUESTION BANK (25 Science + 20 Current Affairs)
# Each question has 'q', 'options' (list of 4), 'ans' (exact text of correct option)
# -----------------------
RAW_QUESTIONS = [
    # 25 Science
    ...
    {"q": "Which element has the highest electronegativity (Paulings scale)?",
    ...
    # 20 Current Affairs (user-provided)
    ...
    {"q": "COP29 (implemented in 2025 decisions) was hosted by—",
...
# -----------------------
# Build shuffled_questions with options shuffled (and include "Not Attempted")
# -----------------------
def build_shuffled():
    shuffled = []
    for item in RAW_QUESTIONS:
        opts = item["options"].copy()
        # include "Not Attempted" as an option to allow skipping
        # we'll put it as first option so it's easy to spot
        # but still we'll shuffle options — keep "Not Attempted" distinct
...
# -----------------------
# Timer display (JS reload to keep live on Streamlit Cloud)
# -----------------------
if not st.session_state.submitted:
    mins, secs = divmod(remaining, 60)
    timer_html = f"""
    <div style="font-size:20px; font-weight:600; color:#b30000;">
...
if not st.session_state.submitted:
    ...
    for i, q in enumerate(QUESTIONS):
        # Pre-populate responses if user navigates back (persisted in session_state.responses)
        default = st.session_state.responses.get(i, None)
        # show radio; key must be unique and constant across reruns
        choice = st.radio(f"Q{i+1}. {q['q']}", q["options"], index= q["options"].index(default) if default in q["options"] else None, key=f"q_{i}")
...
if st.session_state.submitted:
    ...
    # Build per-question rows for CSV
    rows = []
    for i, q in enumerate(QUESTIONS):
    ...
    writer = csv.DictWriter(csv_buffer, fieldnames=["Q_no", "Question", "Your Answer", "Correct Answer", "Outcome", "Marks"])
    writer.writeheader()
    for r in rows:
        writer.writerow(r)
    csv_bytes = csv_buffer.getvalue().encode("utf-8")

    st.download_button("Download detailed result (CSV)", data=csv_bytes,
